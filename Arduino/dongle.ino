...
struct __attribute__((packed)) PersistedState {
  ...
  char passwordCombo[PASSWORD_BUFFER_SIZE];
  uint8_t passwordIsSet;
  uint8_t checksum;
};
...
uint8_t calcPersistChecksum(const PersistedState &state) {
  ...
  uint8_t checksum = 0;
  for (size_t i = 0; i < sizeof(PersistedState) - 1; ++i) {
    checksum ^= bytes[i];
  }
  return checksum;
}
...
void saveStateToEeprom() {
  ...
  memcpy(state.passwordCombo, g_passwordCombo, PASSWORD_BUFFER_SIZE);
  state.passwordCombo[PASSWORD_BUFFER_SIZE - 1] = '\0';
  state.passwordIsSet = g_passwordIsSet ? 1 : 0;
  state.checksum = calcPersistChecksum(state);
...
bool loadStateFromEeprom() {
  ...
  if (state.magic != EEPROM_MAGIC || state.version != EEPROM_VERSION) {
    return false;
  }
  if (state.checksum != calcPersistChecksum(state)) {
    return false;
  }

  g_settings = state.settings;
  memcpy(g_passwordCombo, state.passwordCombo, PASSWORD_BUFFER_SIZE);
  g_passwordCombo[PASSWORD_BUFFER_SIZE - 1] = '\0';
  g_passwordIsSet = (state.passwordIsSet != 0);
  g_isAuthenticated = !g_passwordIsSet;
  rebuildButtonMap();
  return true;
}
